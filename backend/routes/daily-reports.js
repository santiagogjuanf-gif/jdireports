...
// (Para �rdenes de tipo post-construcci�n)
// ================================================
...
const createDailyReportValidation = [
  body('report_date')
    .isISO8601()
    .withMessage('Fecha del reporte debe ser v�lida'),
  body('description')
    .trim()
...
router.post('/orders/:orderId/reports', authenticateToken, requireRole(['trabajador']), createDailyReportValidation, handleValidationErrors, async (req, res) => {
  try {
    ...
    // Verificar que la orden existe y es de tipo post-construcci�n
    ...
    if (order.order_type !== 'post_construction') {
      throw new ConflictError('Los reportes diarios solo est�n disponibles para �rdenes de post-construcci�n');
    }

    // Verificar que el trabajador est� asignado a esta orden
...
router.get('/reports/:reportId', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('📸 [DAILY-REPORTS] Obteniendo fotos para reporte:', reportId);
    ...
    `, [reportId]);
    console.log('📸 [DAILY-REPORTS] Fotos encontradas:', photosResult.rows?.length || 0);
...
router.put('/reports/:reportId', authenticateToken, requireRole(['trabajador']), [
  ...
  try {
    ...
    // Verificar que el trabajador es quien cre� el reporte
    if (report.created_by !== workerId) {
      throw new ForbiddenError('Solo puedes editar reportes que t� creaste');
    }

    // No se puede editar si la orden est� completada o cancelada
