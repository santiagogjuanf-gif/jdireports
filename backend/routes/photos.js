...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (validateImageFormat(file.mimetype)) {
      ...
      cb(new Error('Formato de imagen no v�lido. Solo se permiten JPG, PNG y WEBP'));
    }
  }
});
...
router.post('/orders/:orderId/photos', authenticateToken, requireRole(['trabajador']), upload.single('photo'), async (req, res) => {
  try {
    ...
    if (!req.file) {
      throw new ValidationError('No se proporcion� ninguna imagen');
    }
    ...
    // Verificar que el trabajador est� asignado
    ...
    if (!isAssigned) {
      throw new ForbiddenError('No est�s asignado a esta orden');
    }

    // Verificar que la orden est� en progreso
    if (order.status !== 'in_progress') {
      throw new ConflictError('Solo se pueden subir fotos a �rdenes en progreso');
    }

    // Verificar l�mite de fotos seg�n tipo de orden
    ...
    if (photoCount.count >= maxPhotos) {
      throw new ConflictError(`Se ha alcanzado el l�mite de ${maxPhotos} fotos para este ${order.order_type === 'regular' ? 'orden' : 'reporte diario'}`);
    }

    // Validar tama�o
    if (!validateImageSize(req.file.size)) {
      throw new ValidationError('La imagen excede el tama�o m�ximo permitido de 10MB');
    }
    ...
    console.log('📸 [PHOTOS] Guardando foto en BD...');
    const photoData = {
      order_id: orderId,
    ...
    console.log('📸 [PHOTOS] Datos de foto:', photoData);

    const photoId = await insert('order_photos', photoData);
    console.log('✅ [PHOTOS] Foto guardada con ID:', photoId);
...
router.get('/orders/:orderId/photos', authenticateToken, async (req, res) => {
  try {
    ...
    // Construir query
    console.log('📸 [PHOTOS] Obteniendo fotos para orden:', orderId);
    let sql = `
      SELECT
        op.id,
        op.photo_path,
...
router.put('/photos/:photoId/caption', authenticateToken, requireRole(['trabajador']), async (req, res) => {
  try {
    ...
    // Solo el que subi� la foto puede cambiar el caption
    if (photo.uploaded_by !== workerId) {
      throw new ForbiddenError('Solo puedes editar fotos que t� subiste');
    }

    // No se puede editar si la orden est� completada
    if (photo.status === 'completed' || photo.status === 'cancelled') {
      throw new ConflictError('No se puede editar fotos de �rdenes completadas o canceladas');
    }
...
router.delete('/photos/:photoId', authenticateToken, async (req, res) => {
  try {
    ...
    // Solo admin, jefe o el que subi� la foto pueden eliminar
    ...
    // No se puede eliminar si la orden est� completada
    if (photo.status === 'completed') {
      throw new ConflictError('No se pueden eliminar fotos de �rdenes completadas');
    }
